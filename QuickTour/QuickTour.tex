...
% 2012-06-04 - Ben Coman began update to Pharo 1.4
...
\pharo is available as a free \ind{download} from \pharoweb.  For the purpose of working through this book, we recommend you use the \emph{PBE-1.4-OneClick.zip} provided on the Pharo by Example web site.\footnote{\pbe} 
However most of the introductory material in this book will work with any version, so if you already have one installed, you may as well continue to use it.  
If you encounter any surprises by way of missing packages, the appendix describes the build of \emph{PBE-1.4-OneClick} from the standard image.  

The \emph{OneClick.zip} file contains the running system for three platform: Mac, Linux and Windows.  Installation involves merely expanding this zip file to any desired location.

\dothis{Download and install \emph{PBE-1.4-OneClick.zip} on your computer.}
\index{download}
\seclabel{sbeImage}

\newpage
Located under the expanded \emph{Pharo-1.4-one-click.app} folder are the three main parts of Pharo:  
...
\begin{enumerate}
  ...
  \begin{enumerate} 
    \item Mac: \emph{Contents\textbackslash Linux\textbackslash pharo} 
    \item Linux: \emph{Contents\textbackslash MacOS\textbackslash pharo}   
    \item Windows: \emph{CogVM.exe}   
   \end{enumerate}
  

  \item The \emphind{sources} file contains the source code for all the parts of \pharo that don't change very frequently:
  
  \begin{enumerate}
    \item All platforms: \emph{Contents\textbackslash Resources\textbackslash PharoV10.sources}.
  \end{enumerate}

  \item The current \emph{system \ind{image}} which is a snapshot of a running \pharo system, frozen in time.  This consists of two files: an \emph{.}\emphind{image} file, which contains the state of all of the objects in the system (including classes and methods, since they are objects too), and a \emph{.}\emphind{changes} file, which contains a log of all of the changes to the source code of the system:
  
  \begin{enumerate}
    \item All platforms: \emph{Contents\textbackslash Resources\textbackslash Pharo-1.4-one-click.image}
    \item All platforms: \emph{Contents\textbackslash Resources\textbackslash Pharo-1.4-one-click.changes}  
  \end{enumerate}
...
\dothis{Start Pharo using the method appropriate to your platform: }
  \begin{enumerate}
  
    \item Mac: \emph{ \color{red} \#\#\# TO BE DEFINED BY SOMEONE WITH A MAC \#\#\#}
  
    \item Linux: \emph{sh Pharo-1.4-one-click.sh}
    
    \item Windows: Double-click on \emph{CogVM.exe}.  \footnote{The associated \emph{CogVM.INI} file defines the location of which image to run.} 
   
   \end{enumerate}


Once \pharo is running, you should see a single large window, possibly containing some open workspace windows (see \figref{startup})
You can dismiss any open workspaces by \click{ing} on the cross in the top left corner of the workspace window. Alternatively, the icon next to this is a small line which when \click{ed} minimizes windows to the dock at the bottom of screen.  Clicking the hollow square maximises the window to take up the full screen.
...
Now it may not be obvious how to proceed, but \pharo mainly makes use of context-dependent pop-up menus.
...
\index{yellow button}
\index{blue button}
Since many users will use various modifiers keys ({\sc crtl}, {\sc alt}, {\sc meta} \etc) to achieve the same effect, we will instead use the following terms:
\begin{description}
...
either you must hold {\sc shift-ctrl}  or {\sc shift-option} while clicking.
...
\dothis{Type \ct{Time now} in the workspace. Now \actclick the workspace and \menu{print it}.}
...
\pharo has some settings for customising the mouse and the meta keys on your keyboard.
In the Settings Browser (\menu{World {\go} System {\go} Settings {\go} Settings Browser...}), the item \menu{System {\go} Keyboard {\go} Control and Alt keys} contains an option that switches the \actclick and \metaclick functions.
There are also options to duplicate the various command keys.   

\begin{figure}[htb]
\centerline{\includegraphics[width=\textwidth]{PreferenceBrowser}}
\caption{The Settings Browser.\figlabel{prefBrowser}}
\end{figure}
...
You will see a list of several of the core tools in \pharo, including the System Browser and the Workspace.
...
A window containing a large number of bouncing atoms should open in the top left of the \pharo image. (\figref{atoms})
...
The \bam class decided what to do with the \ct{new} message, that is, it looked up its \emph{methods} for handling \ct{new} message and reacted appropriately, in this case returning an instance of \bam. 
...
\centerline {\includegraphics[width=0.8\textwidth]{atoms}}
\caption{A \bam.\figlabel{atoms}}
\end{minipage}
\hfill
\begin{minipage}[b]{0.48\textwidth}
\centerline {\includegraphics[width=1.0\textwidth]{saveAs}}
...
\dothis{Using the mouse (and the appropriate modifier keys), \metaclick on the \bam. \footnote{Remember, you may have to set the \ct{halosEnabled} option in the Settings Browser. \btc{I can't see this setting in Setting Browser - still valid for 1.4?} } }
...
At any time only one window is active; it is in front, has its border highlighted and its title emphasised.
...
\needlines{2}
\dothis{Close the inspector. Type the expression \ct{BouncingAtomsMorph new} in a workspace and this time \menu{explore it} (\short{I}, uppercased i).}
\index{keyboard shortcut!explore it}
\index{explorer}

This time you should see a window labelled \clsind{a BouncingAtomsMorph} containing the text
\mbox{$\triangleright$ \ct{root: a BouncingAtomsMorph}}.
Click on the triangle to open it up (\figref{exploreit}).

\begin{figure}[htb]
\centerline {\includegraphics[width=0.9\textwidth]{ExploreIt}}
\caption{Exploring an instance of \ct{BouncingAtomsMorph}. \figlabel{exploreit}}
\end{figure}

The explorer is similar to the inspector, but it offers a tree view of a complex object.
In this case the object we are looking at an instance of the \ct{BouncingAtomsMorph} class.
We can see directly all the information stored in this class, and we can easily navigate to all its parts.
Again the bottom panel can be used to write expressions to send messages to the object.

\dothis{Type the expression \ct{self openInWorld} in the bottom panel of the explorer, then select this and \menu{do it} }

You will see \figref{atoms} again.
